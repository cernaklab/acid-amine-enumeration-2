...
am3 = Chem.MolFromSmarts("CC[NH2]")
# ac3 = Chem.MolFromSmarts("[CX4H2][CX4H1][CX3](=O)O")
ac3 = Chem.MolFromSmarts("[CX4H0][CX4H1][CX3](=O)O")
...
def run_enumeration(file_tag, acid, amine, starting_numHs,side):
    index_start = file_tag * 1000000
    file_tag = str(file_tag).zfill(2)

    dmats = np.load(f"../rxn_mats/dmats_ac3_am3_{file_tag}.npy")
    out_file = open(f"./finasteride_products/side{side}/finasteride_side{side}_{file_tag}.csv","w")
    ...
    for i, dmat in enumerate(dmats):
        ...
        if all(BO_increase <= starting_numHs):
            ...
            m = apply_amat(acid,"ac3",amine,"am3",dmat)
...
def run_enumeration_demo(file_tag, acid, amine, starting_numHs,side):
    index_start = file_tag * 1000000
    file_tag = str(file_tag).zfill(2)

    dmats = np.load(f"../rxn_mats/dmats_ac3_am3_{file_tag}.npy")
    out_file = open(f"./finasteride_products/side{side}/finasteride_side{side}_demo_{file_tag}.csv","w")
    out_file.write("rmat_tag,pdt_smiles\n")
    
    for i, dmat in enumerate(dmats[::100]):
        BO_increase = sum(dmat)
        if all(BO_increase <= starting_numHs):

            matrix_index = index_start + i*100
            m = apply_amat(acid,"ac3",amine,"am3",dmat)
...
def couple_amide(amine_smiles):
    ...
    amide_pdt = amidecoup.RunReactants((finasteride_acid,amine))
