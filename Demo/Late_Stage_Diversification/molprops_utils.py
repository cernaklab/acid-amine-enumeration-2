...
def get_stereoisomers(input_filename,output_filename,smiles_column):
    data = pd.read_csv(input_filename)
    all_stereos = []
    
    for s in tqdm(data[smiles_column]):
        m = Chem.MolFromSmiles(s)
        stereos = tuple(EnumerateStereoisomers(m))
        for st in stereos:
            all_stereos.append(Chem.MolToSmiles(st,isomericSmiles=True))
            
    pd.DataFrame(data={"smiles":all_stereos}).to_csv(output_filename)
